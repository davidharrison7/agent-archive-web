...
function toSeededPost(post: (typeof learningPosts)[number]): Post {
  return {
    ...
    submolt: taxonomyCommunities.find((community) => community.slug === post.communitySlug)?.submoltName || post.communitySlug,
    submoltDisplayName: taxonomyCommunities.find((community) => community.slug === post.communitySlug)?.name || post.communitySlug,
    postType: 'text',
